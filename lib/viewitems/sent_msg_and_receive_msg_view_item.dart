...
import 'package:we_chat_app/viewitems/voice_msg_view_custom_widget.dart';
        ...
        children: [
          Padding(
            ...
            child: ProfileImgWithActiveStatusView(
              chatUserProfile: msgItem.profileUrl ?? "",
            ),
          ),
          Column(
            mainAxisSize: MainAxisSize.min,
            ...
            children: [
              ...
              Visibility(
                  ...
                  child: ReceiveImgOrVideoMsgView(
                      bloc: bloc,
                      msgItem: msgItem,
                      onTapSelectedPhoto: (url, mediaTypeVO) {
                        return onTapSelectedPhoto(url, mediaTypeVO);
...
class ReceiveImgOrVideoMsgView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    DateTime dateTime = DateTime.fromMillisecondsSinceEpoch(
        int.parse(msgItem.timestamp.toString()));
    String dateString = dateFormat.format(dateTime);

    return (sendFileList.length == 1)
        ? Column(
            crossAxisAlignment: CrossAxisAlignment.end,
            children: [
              GestureDetector(
                onTap: () {
                  return onTapSelectedPhoto(
                      sendFileList[0].fileUrl.toString(), sendFileList[0]);
                },
                child: Container(
                  width: 160,
                  height: 100,
                  ...
                  child: (sendFileList[0].fileUrl != "")
                      ?
                  (sendFileList[0].fileType.toString().contains("aac"))
                      ? VoiceMsgView(
                      bloc: bloc,
                      msgItem: msgItem,
                      voiceRecordUrl: sendFileList[0].fileUrl ?? ""):
                  (sendFileList[0].fileType.toString().contains("video"))
                          ? VideoMsgView(
                              bloc: bloc,
                              msgItem: msgItem,
                              videoUrl: sendFileList[0].fileUrl ?? "",
                  mediaTypeVO:sendFileList[0])
                          : ClipRRect(
                              borderRadius:
                ...
                GridView.builder(
                  ...
                  itemBuilder: (context, index) {
                    return GestureDetector(
                      onTap: () {
                        return onTapSelectedPhoto(
                            sendFileList[index].fileUrl.toString(),
                            sendFileList[index]);
                      },
                      child: Container(
                        ...
                        child: (sendFileList[index].fileUrl != "")
                            ?
                        (sendFileList[0].fileType.toString().contains("aac"))
                            ? VoiceMsgView(
                            bloc: bloc,
                        ...
                        (sendFileList[index]
                                ...
                                ? VideoMsgView(
                                    bloc: bloc,
                                    msgItem: msgItem,
                                    videoUrl: sendFileList[index].fileUrl ?? "",
                            mediaTypeVO:sendFileList[index]
                                  )
                                : ClipRRect(
...
class ImageMsgOrVideoMsgView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    List<MediaTypeVO> sendFileList =
        msgItem.mediaFile ?? []; //convertToList(msgItem.mediaFile);
            ...
            children: [
              GestureDetector(
                onTap: () {
                  ...
                  onTapSelectedPhoto(
                      sendFileList[0].fileUrl ?? "", sendFileList[0]);
                },
                child: Container(
                  width: 160,
                  height: 100,
                  ...
                  itemBuilder: (context, index) {
                    return GestureDetector(
                      onTap: () {
                        ...
                        onTapSelectedPhoto(sendFileList[index].fileUrl ?? "",
                            sendFileList[index]);
                      },
                      child: Container(
                        width: IMAGE_MESSAGE_WIDTH,
...
class VideoMsgView extends StatelessWidget {
  ...
  const VideoMsgView(
      ...
      required this.mediaTypeVO});

  final ChatMessageVO msgItem;

  @override
  Widget build(BuildContext context) {
    return VideoViewCustomWidget(mediaTypeVO:mediaTypeVO,videoUrl: videoUrl,isExitFullScreen: false,);
  }
}
class VoiceMsgView extends StatelessWidget {
  ...
  const VoiceMsgView(
      {super.key,
        required this.msgItem,
        required this.bloc,
        required this.voiceRecordUrl});

  final ChatMessageVO msgItem;

  @override
  Widget build(BuildContext context) {
    return VoiceMsgViewCustomWidget(voiceRecordUrl: voiceRecordUrl);
  }
}
